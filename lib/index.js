...
class FFContextStorage {
    ...
    get (scope, key, callback) {
        if (typeof callback !== 'function') {
            ...
            return Promise.reject(new Error('This context store must be called asyncronously'))
        }
        const path = `${scope}`
        ...
        }).catch(error => { // TODO: If resourece not found, return null or error?
    ...
    set (scope, key, value, callback) {
        if (typeof callback !== 'function') {
            // TODO: consider adding a cache to permit synchronous calls
            throw new Error('This context store must be called asyncronously')
        }
        const path = `${scope}`
